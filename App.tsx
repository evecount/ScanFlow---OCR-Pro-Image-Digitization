...
const App: React.FC = () => {
  ...
  const renderPdfPageToImage = async (pdf: any, pageNumber: number): Promise<string> => {
  ...
  const processIncomingFiles = useCallback(async (fileList: File[]) => {
    for (const file of fileList) {
      if (file.type === 'application/pdf') {
        try {
          ...
          // No maximum pages - process everything
          for (let i = 1; i <= pdf.numPages; i++) {
            const preview = await renderPdfPageToImage(pdf, i);
            const newScannedFile: ScannedFile = {
              id: Math.random().toString(36).substring(2, 11),
              file,
              displayName: pdf.numPages > 1 ? `${file.name} (P. ${i}/${pdf.numPages})` : file.name,
              preview,
            ...
            setFiles(prev => {
              const updated = [...prev, newScannedFile];
              // Auto-detect regions if this is the very first page and no regions exist
              if (updated.length === 1 && regions.length === 0) {
                triggerAutoDetectForFile(newScannedFile);
              }
              return updated;
            });
          }
  ...
  const processAllFiles = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const syncSuccess = await saveExtractionToFirestore(batchId, targetFile.displayName || targetFile.file.name, result, regions);
        setFiles(prev => prev.map((f, idx) => 
  ...
  const exportAsCSV = () => {
    ...
    const headers = ['File/Page Name', ...regions.map(r => r.name)];
    const csvRows = completedFiles.map(f => [
      f.displayName || f.file.name,
    ...
    link.setAttribute('download', `scanflow_batch_${batchId}.csv`);
  ...
  const syncToGoogleSheets = async () => {
    ...
    for (const file of files) {
      if (file.status === 'completed' && file.extractedData) {
        const rowData = [file.displayName || file.file.name, ...regions.map(r => file.extractedData?.[r.name] || '')];
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden text-slate-900 relative">
      {/* Full-screen Drag Overlay */}
      {isDragging && (
        <div className="absolute inset-0 z-[100] bg-indigo-600/90 backdrop-blur-sm flex flex-col items-center justify-center text-white p-10 text-center">
          <div className="w-24 h-24 bg-white/20 rounded-full flex items-center justify-center mb-6 animate-bounce">
            <Upload size={48} />
          </div>
          <h2 className="text-4xl font-bold mb-2">Drop your files</h2>
          <p className="text-xl opacity-80 max-w-md">Release to add PDFs or images to the processing queue.</p>
        </div>
      )}
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-200 z-10 shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Multi-Page Automation Engine</p>
          </div>
        </div>
            ...
            <div className="w-80 border-r border-slate-200 bg-white flex flex-col">
              <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{files.length} items</span>
                </div>
                <label 
                  className={`flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-xl cursor-pointer group transition-all border-slate-300 hover:border-indigo-400 hover:bg-indigo-50`}
                >
                  <Upload className={`mb-1 transition-colors text-slate-400 group-hover:text-indigo-500`} size={20} />
                  <span className={`text-[10px] font-bold transition-colors text-slate-500 group-hover:text-indigo-600`}>
                    Browse or Drop PDFs
                  </span>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {files.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                    ...
                    <p className="text-xs font-medium text-slate-400">Your processing queue is currently empty.</p>
                  </div>
                )}
                {files.map((file, idx) => (
                  <div key={file.id} onClick={() => setCurrentIndex(idx)} className={`group relative p-3 rounded-xl border transition-all cursor-pointer ${currentIndex === idx ? 'border-indigo-200 bg-indigo-50 ring-1 ring-indigo-100' : 'border-slate-100 hover:border-slate-200 bg-white'}`}>
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-slate-700 truncate pr-4">{file.displayName || file.file.name}</p>
            ...
            <div className="flex-1 bg-slate-100/50 p-8 flex flex-col items-center relative overflow-y-auto">
              {currentFile ? (
                <div className="w-full max-w-3xl space-y-4">
                  <div className="flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-widest">
                    <div className="flex items-center space-x-4">
                      ...
                      <button 
                        onClick={() => triggerAutoDetectForFile(currentFile)} 
                        disabled={isDetecting}
                        className="text-indigo-600 flex items-center space-x-1 hover:text-indigo-700 transition-colors disabled:opacity-50"
                      >
                        {isDetecting ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12}/>}
                        <span>{isDetecting ? 'Detecting...' : 'Recalibrate Page'}</span>
                      </button>
                    </div>
                    <span>{currentFile.displayName || currentFile.file.name}</span>
                  </div>
                  <DocumentCanvas 
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-slate-500 text-sm max-w-xs mt-2">Drop a PDF or image anywhere on this screen to start your batch process.</p>
                </div>
              )}
            </div>
            ...
            <div className="w-80 border-l border-slate-200 bg-white flex flex-col">
              ...
              <div className="p-4 border-b border-slate-100">
                <textarea 
                  ...
                  placeholder="Example: 'Find the total amount including tax' or 'Ignore handwritten notes'..."
              ...
              <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-slate-700 flex items-center space-x-2"><Settings size={16} className="text-indigo-500" /><span>Extraction Fields</span></h3>
                  <button 
                    onClick={() => {
              ...
              <div className="p-4 flex-1 overflow-y-auto space-y-3">
                {regions.length === 0 && (
                  <div className="text-center py-6 px-4">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">No Fields Defined</p>
                    <p className="text-[9px] text-slate-400 mt-1">Draw directly on the document to add a field.</p>
                  </div>
                )}
                {regions.map((region) => (
          ...
          <div className="flex-1 flex flex-col bg-white overflow-hidden p-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-slate-500 text-sm">Every scan is automatically persisted to Firestore.</p>
              </div>
              <div className="flex items-center space-x-3">
                <button 
                  onClick={exportAsCSV}
                  className="flex items-center space-x-2 px-6 py-2 bg-slate-100 text-slate-700 rounded-xl font-bold text-xs hover:bg-slate-200 transition-all active:scale-95 shadow-sm border border-slate-200"
                >
                  <Download size={16} />
                ...
                <button onClick={() => setView('editor')} className="text-xs font-bold text-indigo-600 hover:underline">Return to Editor</button>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase sticky top-0 bg-slate-50">Document Source</th>
                    <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase text-center sticky top-0 bg-slate-50">Cloud Sync</th>
                    {regions.map(r => <th key={r.id} className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase sticky top-0 bg-slate-50">{r.name}</th>)}
                  </tr>
                </thead>
                  ...
                  {files.map(file => (
                    <tr key={file.id} className={`${file.status === 'error' ? 'bg-red-50/50' : 'hover:bg-slate-50/50 transition-colors'}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <div className="w-6 h-8 bg-slate-100 rounded border flex-shrink-0 overflow-hidden">
                            <img src={file.preview} className="w-full h-full object-cover" />
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs font-bold text-slate-700">{file.displayName || file.file.name}</span>
      ...
      <footer className="px-6 py-3 bg-white border-t border-slate-200 flex items-center justify-between">
        <div className="flex items-center space-x-6">
          <div className="flex items-center space-x-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            ...
            <span>System Status: {isProcessing ? 'Processing Batch' : 'Standby'}</span>
          </div>
          <div className="flex items-center space-x-2 border-l border-slate-200 pl-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            ...
            <span>Persistence: Firestore Active</span>
          </div>
        </div>
        <div className="flex items-center space-x-4 text-[10px] font-bold text-slate-400">
          ...
          <span className="bg-slate-100 px-2 py-0.5 rounded">Batch ID: {batchId}</span>
        </div>
      </footer>
    </div>
  );
